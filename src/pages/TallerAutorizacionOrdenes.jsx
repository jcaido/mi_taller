...
import {
  ...
  obtenerOrdenesReparacionAbiertasPorFechaApertura,
  obtenerOrdenesReparacionAbiertasPorVehiculo,
...
function TallerAutorizacionOrdenes() {
    ...
    switch (action.type) {
      case 'ordenReparacion':
        return {
          ...
          tablaOrdenesReparacionAbiertasBusquedas:
          action.payload.tablaOrdenesReparacionAbiertasBusquedas,
  ...
  const [state, dispatch] = useReducer(
    autorizacionOrdenesReducer,
    autorizacionOrdenReparacionInicial,
  );
  ...
  function buscarOrdenReparacionParaEliminarDispatch() {
    dispatch({
      type: 'ordenReparacion',
      payload: {
        formNuevaOrdenReparacion: false,
    ...
    obtenerVehiculosPorMatricula(matricula)
      .then((response) => {
        obtenerOrdenesReparacionAbiertasPorVehiculo(response.data.id)
          .then((resp) => {
            dispatch({ type: 'actualizar_lista_ordenes_reparacion_abiertas', payload: resp.data });
          })
          .then(() => {
  ...
  const AutorizacionOrdenesProviderValue = useMemo(
    () => ({
      state,
  ...
  return (
    <AutorizacionOrdenesContext.Provider value={AutorizacionOrdenesProviderValue}>
      <Grid container>
        <Grid item xs={12}>
