...
import {
  ...
  obtenerCodigosPostalesPorLocalidad,
  obtenerCodigosPostalesPorProvincia,
  obtenerPropietarioPorDni,
  obtenerPropietarios,
  obtenerPropietariosPorCodigoPostal,
  obtenerPropietariosPorPrimerApellido,
  obtenerVehiculos,
  obtenerVehiculosPorMatricula,
  obtenerVehiculosPorMarcaModelo,
...
function DatosGenerales() {
    ...
    obtenerCodigoPostalPorCodigo(codigo)
      .then((response) => {
        obtenerPropietariosPorCodigoPostal(response.data.id)
          .then((resp) => {
            dispatch({ type: 'actualizar_lista_propietarios_por_codigo_postal', payload: resp.data });
          })
          .then(() => {
    ...
    obtenerPropietarioPorDni(dni)
      .then((response) => {
        obtenerVehiculosPorPropietario(response.data.id)
          .then((resp) => {
            dispatch({ type: 'actualizar_lista_vehiculos_por_propietario', payload: resp.data });
          })
          .then(() => {
  ...
  const DatosGeneralesProviderValue = useMemo(
    () => ({
      state,
  ...
  return (
    <DatosGeneralesFormContext.Provider value={DatosGeneralesProviderValue}>
      <Grid container>
        <Grid item xs={12}>
          <Box>
            <NavBar
              pages={['INICIO', 'TALLER', 'ALMACÉN', 'FACTURACION', 'CONTABILIDAD']}
