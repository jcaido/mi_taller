      ...
      <Card sx={{ minWidth: 275 }}>
        <CardContent>
          <Typography sx={{ fontSize: 14 }} color="text.secondary" gutterBottom>
            CODIGO:
            {' '}
            {state.codigoPostalPorCodigo && state.listaCodigosPostalesPorCodigo.codigo}
            {state.codigoPostalPorLocalidad && state.listaCodigosPostalesPorLocalidad.codigo}
          </Typography>
          <Typography sx={{ fontSize: 14 }} color="text.secondary" gutterBottom>
            LOCALIDAD:
            {' '}
            { state.codigoPostalPorCodigo && state.listaCodigosPostalesPorCodigo.localidad }
            {state.codigoPostalPorLocalidad && state.listaCodigosPostalesPorLocalidad.localidad}
          </Typography>
          <Typography sx={{ fontSize: 14 }} color="text.secondary" gutterBottom>
            PROVINCIA:
            {' '}
            { state.codigoPostalPorCodigo && state.listaCodigosPostalesPorCodigo.provincia }
